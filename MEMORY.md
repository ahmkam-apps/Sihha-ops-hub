...
_**2026-08-06 current hardening status:** non-paid security/workflow batch `94cbfbf` is live on staging and production. A second local batch adds DB-backed readiness/throttling, transactional family deletion with financial-history preservation, fail-closed order validation, expiring single-use legacy confirmation tokens, and verified/quota-controlled uploads with orphan cleanup. Current verification: 208 passed, 1 intentional live-smoke skip. Paid receipt/payment invariants remain deliberately deferred. Treat `CLAUDE.md` as the current schema/route reference._
